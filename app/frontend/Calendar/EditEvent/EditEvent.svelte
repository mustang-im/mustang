...
<vbox flex class="event-edit-window">
  ...
  <hbox flex class="left-right">
    ...
    <vbox flex class="right-pane">
      ...
      {/if}
      <hbox>{JSON.stringify({ frequency, interval, count, weekdays, week, end, endDate }, null, 1)}</hbox>
...
<script lang="ts">
  ...
  let weekdays = event.recurrenceRule?.weekdays || [];
  let week = event.recurrenceRule?.week || 0;
  let end = event.recurrenceRule?.untilDate ? "date" : Number.isFinite(event.recurrenceRule?.count) ? "count" : "none";
  let endDate = event.startTime;
...
<style>
  ...
  grid.time :global(input[type="date"]) {
    max-width: none;
  }
  .duration {

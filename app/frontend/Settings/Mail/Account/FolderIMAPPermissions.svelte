<fieldset>
  ...
  <hbox>
    ...
    <vbox>
      <label><input type=checkbox on:click={() => setPermission("k")} bind:checked={person.subfolder}/> {$t`New Subfolder`}</label>
      <label><input type=checkbox on:click={() => setPermission("x")} bind:checked={person.remove}/> {$t`Remove Folder`}</label>
      <label><input type=checkbox on:click={() => setPermission("t")} bind:checked={person.delete}/> {$t`Delete Messages`}</label>
      <label><input type=checkbox on:click={() => setPermission("e")} bind:checked={person.expunge}/> {$t`Expunge`}</label>

...
test.skipIf(!Database)("Group chat: join, members, history, send, live, restart", { timeout: 60000 }, async () => {
  ...
  let fromAlice = room1.messages.find(msg => msg.id == "msg-g1");
  expect(fromAlice.contact).toBe(alice);
  ...
  await new Promise(resolve => setTimeout(resolve, 500));
  let live = room1.messages.find(msg => msg.text == "Live boo");
  expect(live).toBeTruthy();
  expect(live.contact).toBe(anon);
  ...
  let fromAnon = room2.messages.find(msg => msg.id == "msg-g2");
  expect(fromAnon.contact?.name).toBe("anon");

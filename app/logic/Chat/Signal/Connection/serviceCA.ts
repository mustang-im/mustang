/** Signal's service root CA (subject = issuer = "Signal Messenger, LLC / Signal
 * Messenger"; valid 2022-01-26 … 2032-01-24), extracted from Signal-Android
 * `app/src/main/res/raw/whisper.store`.
 *
 * Signal's service hosts (chat / storage / cdn / cdsi / sfu) present certificates
 * that root in this PRIVATE CA, not a public one, so default TLS rejects them. Both
 * sides that talk to Signal must be told to trust it: the renderer's Chromium TLS
 * for REST (desktop `signalServiceCA.ts`, via `setCertificateVerifyProc`) and the
 * Node backend `ws` socket for the chat WebSocket (backend `newWebSocket`, via the
 * `ca` TLS option). Kept here so both reference one copy. */
export const kSignalServiceRootCA = `-----BEGIN CERTIFICATE-----

9qfgAJizyy3kqM4lOwBH
-----END CERTIFICATE-----
`;

...
export class OWAEvent extends Event {
  ...
  async saveCalendarItem() {
    ...
    if (this.isIncomingMeeting) {
      ...
      await this.calendar.account.callOWA(request, this.calendar.mailbox);
      return;
    }
    ...
    let response = await this.calendar.account.callOWA(request, this.calendar.mailbox);
  ...
  protected async getOnlineMeetingDescription() {
    let response = await this.calendar.account.callOWA(owaOnlineMeetingDescriptionRequest([ this.itemID ]), this.calendar.mailbox);
  ...
  protected async getOnlineMeetingURL() {
    let response = await this.calendar.account.callOWA(owaOnlineMeetingURLRequest([ this.itemID ]), this.calendar.mailbox);
    this.onlineMeetingURL = sanitize.url(response.Items[0].OnlineMeetingJoinUrl, null);
  }

  protected async updateUID() {
    let response = await this.calendar.account.callOWA(owaGetEventUIDsRequest([ this.itemID ]), this.calendar.mailbox);
  ...
  async deleteFromServer() {
    if (this.itemID) {
      ...
      await this.calendar.account.callOWA(owaCreateExclusionRequest(this, this.parentEvent), this.calendar.mailbox);
    }
  }
  ...
  async fetchFromServer(): Promise<OWAEvent> {
    ...
    let result = await this.calendar.account.callOWA(owaGetEventsRequest([this.itemID]), this.calendar.mailbox);
    let item = result.Items[0];
    if (item.IsOnlineMeeting) {
      let result = await this.calendar.account.callOWA(owaGetCalendarEventsRequest([this.itemID]), this.calendar.mailbox);
  ...
  async makeExclusions(exclusions: OWAEvent[]) {
    await this.calendar.account.callOWA(owaCreateMultipleExclusionsRequest(exclusions, this), this.calendar.mailbox);

...
export class SQLFile {
  static async save(file: File): Promise<void> {
    ...
    if (!file.dbID) {
      let insert = await (await getDatabase()).run(sql`
        INSERT INTO file (
          directoryID, name, path, size, mimetype, lastMod, syncState, serverURL
        ) VALUES (
          ${directory.dbID}, ${file.name}, ${file.path}, ${file.size},
  ...
  static async read(dbID: number, file: File): Promise<File> {
    let row = await (await getDatabase()).get(sql`
      SELECT name, path, size, mimetype, lastMod, syncState, serverURL
  ...
  static async readAll(directory: Directory): Promise<void> {
    ...
    let rows = await (await getDatabase()).all(sql`
      SELECT id, name, path, size, mimetype, lastMod, syncState, serverURL

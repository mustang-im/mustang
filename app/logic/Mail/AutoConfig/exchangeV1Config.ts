...
import { newExchangeConfig } from "./exchangeV2Config";
...
export async function exchangeAutoDiscoverV1XML(domain: string, emailAddress: string, username: string | null, password: string, abort: AbortController): Promise<ArrayColl<MailAccount>> {
  ...
  let priorityOrder = new PriorityAbortable(abort, [
    exchangeAutoDiscoverV1XMLSingleFetch(url1, callArgs, abort),
    exchangeAutoDiscoverV1XMLSingleFetch(url2, callArgs, abort),
...
export async function exchangeAutoDiscoverV1XMLSingleFetch(url: URLString, callArgs: any, abort: AbortController): Promise<ArrayColl<MailAccount>> {
...
async function fetchV1HTTP(url: URLString, callArgs: any, emailAddress: string, abort: AbortController): Promise<ArrayColl<MailAccount>> {
  ...
  if (redirectDomain == originalDomain) {
    return await exchangeAutoDiscoverV1XMLSingleFetch(redirectURL, callArgs, abort);
  }
    ...
    try {
      ...
      if (protocol == "WEB") {
        ...
        if (owaURL && !accounts.find(acc => acc.url == owaURL && acc.protocol == "owa")) {
          accounts.add(newExchangeConfig(owaURL, "owa", "autodiscover-xml"));
        }
        if (ewsURL && !accounts.find(acc => acc.url == ewsURL && acc.protocol == "ews")) {
          accounts.add(newExchangeConfig(ewsURL, "ews", "autodiscover-xml"));
        }
...
function setSTMPServer(accounts: ArrayColl<MailAccount>) {
  let smtpAccounts = accounts.filterOnce(acc => acc.protocol == "smtp");
  let imapPOPAccounts = accounts.filterOnce(acc => acc.protocol == "imap" || acc.protocol == "pop3");

...
export class ActiveSyncEMail extends EMail {
  ...
  getUpdateCalendars(): Collection<Calendar> {
    ...
    if (this.scheduling == Scheduling.Request) {
      ...
      return appGlobal.calendars.filter(calendar => calendar.protocol == "calendar-activesync" && calendar.url.startsWith(this.folder.account.url + "?") && calendar.username == this.folder.account.username).slice(0, 1);
    }
    return appGlobal.calendars.filter(calendar => calendar.protocol == "calendar-activesync" && calendar.url.startsWith(this.folder.account.url + "?") && calendar.username == this.folder.account.username && calendar.events.some(event => event.calUID == this.event.calUID));
  }

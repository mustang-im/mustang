...
export class JMAPEMail extends EMail {
  ...
  set uid(val: number | null) {
    assert(val === null || typeof (val) == "number", "JMAP EMail UID must be a number");
    this.pID = val;
  }
  ...
  fromFlow(msgInfo: any) {
    ...
    // <https://jmapflow.com/global.html#FetchMessageObject>
    this.uid = sanitize.integer(msgInfo.uid, null);
    this.seq = sanitize.integer(msgInfo.seq, null);
    if (this.downloadComplete) {
      return;
    }
    // <https://jmapflow.com/global.html#MessageEnvelopeObject>
    ...
    this.threadID = sanitize.string(env.threadId, null); // Only if server supports OBJECTID or X-GM-EXT-1 JMAP extension

...
export class SMIMEPrivateKey extends SMIMEPublicKey implements PrivateKey {
  ...
  async decryptKey(): Promise<RSAPrivateKey> {
    return RSAPrivateKey.decode(PrivateKeyInfo.decode(await this.decryptPKCS8()).privateKey);
  }

  async decryptCryptoKey(): Promise<CryptoKey> {
    return crypto.subtle.importKey("pkcs8", await this.decryptPKCS8(), { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" }, true, ["sign"]);
  }

  /** Decrypts our private key with our passphrase.
   * @returns the key in PKCS#8 form */
  protected async decryptPKCS8(): Promise<Uint8Array> {
    ...
    if (this.passphrase) {
      ...
      privateKey = await decryptPBES2(encryptedKey.encryptedData, encryptedKey.encryptionAlgorithm.parameters, this.passphrase);
    } else {
      privateKey = Any.decodePEM(this.privateKeyArmored, { label: "PRIVATE KEY" });
    }
    if (PrivateKeyInfo.decode(privateKey).privateKeyAlgorithm.algorithm != "rsaEncryption") {

...
perl -p -i \
  ...
  ./logic/build.ts
git commit package.json ../desktop/package.json ../mobile/package.json ./logic/build.ts -m "Version $VERSION"
...
perl -p -i \
  ...
  ./logic/build.ts
git commit package.json ../desktop/package.json ./logic/build.ts -m "Starting version $NEXTVERSION"

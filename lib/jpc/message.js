  ...
  async _incomingMessage(message) {
    ...
    try {
      ...
      try {
        ...
        consoleError(ex);
        let objects = new Set(); // For circular reference detection.
        this.send({
          id: message.id,
          success: false,
          message: ex.message,
          ...
          ...JSON.parse(JSON.stringify(ex, function(key, value) {
        ...
        console.error(exInEx);
        // Try again without the original exception object
        try {
          this.send({
            id: message.id,
            success: false,

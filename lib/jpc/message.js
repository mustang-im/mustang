...
export default class MessageCall  {
  ...
  _response(message) {
    ...
    if (message.success) {
      ...
      // Electron console ignores the message, instead assuming that the
      // first line of the stack contains "Error" + the message, if any.
      ex.stack = callWaiting.stack.replace(/^Error.*/, "Error: " + message.message);

...
export default class MailSQLDatabase extends MailDatabase {
  ...
  async migrate() {
    let migrationFilenames = await findMigrationFilenames('./mail-sql/')
    let migrationFiles = await map(migrationFilenames, readMigrationFile);
    migrate(this._db, migrationFiles);
  }
